...
class TwoDimensionsScreen:
    def __init__(self, window: tk.Tk, ogl_frame) -> None:
        ...
        # Botão para Escala
        btn_scale = tk.Button(self.frame, text="Aplicar Escala", command=lambda: ogl_frame.escala(entry_sx.get_value(), entry_sy.get_value()))
        ...
        # Botão para Rotação
        btn_rotate = tk.Button(self.frame, text="Aplicar Rotação", command=lambda: ogl_frame.rotacao(entry_rot.get_value()) )
        ...
        btn_shear = tk.Button(self.frame, text="Aplicar Cisalhamento", command=lambda: ogl_frame.cisalhamento(entry_a.get_value(), entry_b.get_value()))
